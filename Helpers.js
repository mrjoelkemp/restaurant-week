...
function after (times, cb) {
  if (typeof after._times === 'undefined') after._times = 0;

  return function () {
    after._times++;

    if (after._times === times) {
      after._times = 0;
      cb && cb();
    }
  };
...
module.exports.generateResultsPage = function () {
  // Domify the index template
  var $ = cheerio.load(fs.readFileSync('_indexTemplate.html').toString()),
      $searchResults = $('.search-results');

  this.getRestaurantYelpObjects(function (yelpObjects) {

    // Only keep the results with a rating of 4.0 or higher
    // because, who wants to eat at a shitty restaurant?
    yelpObjects = yelpObjects.filter(function (rest) {
      return rest.rating >= 4.0;
    });
    ...
    yelpObjects.forEach(function (o) {
      $searchResults.append(o.html);
    });
...
module.exports.getRestaurantYelpObjects = function (cb) {
  var names = this.getRestaurantNames(),
      yelpObjects = [],
  ...
  names.slice(0, howMany).forEach(function (rName) {
    var yres = new YelpResult(rName);

    yelpObjects.push(yres);

    yres.fetch(whenAllFetched);
  });
};

module.exports.getRestaurantNames = function () {
  return fs.readFileSync(restaurantNamesFile).toString().split('\n');
};

// One-offs used to prep the data
module.exports.saveRestaurantsPage = function () {
...
module.exports.saveRestaurantNames = function () {
